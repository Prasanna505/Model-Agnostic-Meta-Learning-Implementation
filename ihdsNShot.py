...
class ihdsNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz):
        ...
        self.resize = imgsz
        if not os.path.isfile(os.path.join(root, 'ihds3.npy')):
            # if root/data.npy does not exist, just download it
            self.x = ihds(root, download=False,
                              transform=transforms.Compose([lambda x: Image.open(x).convert('L'),
                                                            lambda x: x.resize((imgsz, imgsz)),
                                                            lambda x: np.reshape(x, (imgsz, imgsz, 3)),
                                                            #lambda x: np.transpose(x, [2, 0, 1]),
                                                            lambda x: x/255.])
                              )

            temp = dict()  # {label:img1, img2..., 20 imgs, label2: img1, img2,... in total, 45 label}
            ...
            self.x = []
            for label, imgs in temp.items():  # labels info deserted , each label contains 60imgs
                self.x.append(np.array(imgs))

            # as different class may have different number of imgs
            self.x = np.array(self.x).astype(np.float)  # [[60 imgs],..., 45 classes in total]
            # each character contains 60 imgs
            print('data shape:', self.x.shape)  # [50, 60, 84, 84, 1]
            temp = []  # Free memory
            # save all dataset into npy file.
            np.save(os.path.join(root, 'ihds3.npy'), self.x)
            print('write into ihds3.npy.')
        else:
            # if data.npy exists, just load it.
            self.x = np.load(os.path.join(root, 'ihds3.npy'))
            print('load from ihds3.npy.')
        ...
        self.n_cls = self.x.shape[0]  # 50
        self.n_way = n_way  # n way
        self.k_shot = k_shot  # k shot
        self.k_query = k_query  # k query
        assert (k_shot + k_query) <=20
        #print(self.batchsz,self.n_cls,self.n_way,self.k_shot,self.k_query)
    ...
    def load_data_cache(self, data_pack):
        ...
        :param data_pack: [cls_num, 20, 224, 224, 3]
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                selected_cls = np.random.choice(data_pack.shape[0], self.n_way, True)
                ...
                x_spt = np.array(x_spt).reshape(self.n_way * self.k_shot, 1, self.resize, self.resize)[perm] ##Change
...
if __name__ == '__main__':
    ...
    # plt.ion()
    viz = visdom.Visdom(env='ihds_view')

    db = ihdsNShot('db/ihds', batchsz=20, n_way=5, k_shot=5, k_query=15, imgsz=64)
